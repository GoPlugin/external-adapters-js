# Changelog Plugin External Adapters
...
```
docker pull public.ecr.aws/plugin/adapters/1forge-adapter:1.0.0
```
...
For more details see the public node operator guide [here](https://www.notion.so/plugin/EA-0-3-0-Migration-Guide-3ac0632093a94f4d9ad7ea9e4fab2693).
...
- Moves re-usable test behaviors & testing utils to a new package - `@plugin/adapter-test-helpers`
- Added support for using query string parameters as input to adapters.
- Add a package re-usable adapter factory implementations to - `@plugin/ea-factories`
...
- Migrates @plugin/ea-bootstrap and @plugin/external-adapter packages to TS.
...
- New adapters:
  ...
  - `orchid-bandwidth` to get available bandwidth from Plugin as service provider.
