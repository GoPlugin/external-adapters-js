...
import { Config, ExecuteWithConfig, ExecuteFactory, InputParameters } from '@plugin/ea-bootstrap'
import { Requester, Validator, AdapterError } from '@plugin/ea-bootstrap'
...
// Convert the payment in $PLI into Agoric's pegged $PLI token.
...
const tryExecuteLogError =
  ...
  async (request, context, config) => {
    try {
      ...
      // See https://github.com/goplugin/external-adapters-js/issues/204
