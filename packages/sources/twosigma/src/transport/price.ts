...
import { makeLogger, ProviderResult } from '@plugin/external-adapter-framework/util'
import { WebSocketTransport } from '@plugin/external-adapter-framework/transports'
import { EndpointContext } from '@plugin/external-adapter-framework/adapter'
import { SubscriptionDeltas } from '@plugin/external-adapter-framework/transports/abstract/streaming'
...
export const options = {
  url: (context: EndpointContext<WsTransportTypes>): string => {
    return context.adapterSettings.WS_API_ENDPOINT
  },

  handlers: {
    message: (message: WebSocketMessage): ProviderResult<WsTransportTypes>[] | undefined => {
...
export class TwoSigmaWebsocketTransport extends WebSocketTransport<WsTransportTypes> {
...
export const transport = new TwoSigmaWebsocketTransport(options)
