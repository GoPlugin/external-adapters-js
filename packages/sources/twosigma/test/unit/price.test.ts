...
import { metrics } from '@plugin/external-adapter-framework/metrics'
import { SubscriptionDeltas } from '@plugin/external-adapter-framework/transports/abstract/streaming'
import { WebSocketClassProvider } from '@plugin/external-adapter-framework/transports/websocket'
import { LoggerFactoryProvider, sleep } from '@plugin/external-adapter-framework/util'
...
describe('Config', () => {
  describe('url', () => {
    const context = {
      adapterSettings: {
        WS_API_ENDPOINT: 'wss://plugin.twosigma.com',
      },
      endpointName: 'price',
