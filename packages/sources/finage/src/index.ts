...
import { IncludesFile } from '@plugin/external-adapter-framework/adapter'
...
import { PriceEndpointInputParametersDefinition } from '@plugin/external-adapter-framework/adapter/price'
import { AdapterParams } from '@plugin/external-adapter-framework/adapter/types'
import {
  AdapterRequest,
  AdapterRequestContext,
  AdapterResponse,
} from '@plugin/external-adapter-framework/util'
import { CustomSettingsDefinition } from '@plugin/external-adapter-framework/config'
...
class FinageAdapter<T extends CustomSettingsDefinition> extends PriceAdapter<T> {
  ...
  override async handleRequest(
    ...
    /* 
    overrides PriceAdapter.handleRequest [https://github.com/goplugin/ea-framework-js/blob/main/src/adapter/price.ts#L160] 
