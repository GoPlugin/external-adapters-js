...
import type { Config, ExecuteWithConfig, InputParameters } from '@plugin/ea-bootstrap'
...
export const execute: ExecuteWithConfig<Config> = async (request, _, config) => {
  ...
  if (splitName.length < 2)
    throw new AdapterInputError({
      jobRunID,
      message: `Invalid ENS name. Format must be [domain].[top level domain], (e.g. plugin.eth)`,
      statusCode: 400,
    })
