...
import '@plugin/solana-view-function-adapter'
import '@plugin/token-allocation-adapter'
import type { SuiteContext, EnvVariables } from '@plugin/ea-test-helpers'
import { SuperTest, Test } from 'supertest'

jest.mock('@plugin/solana-view-function-adapter', () => ({
  ...jest.requireActual('@plugin/solana-view-function-adapter'),
  makeExecute: jest.fn().mockReturnValue(() => mockSolanaViewFunctionResponse),
}))

jest.mock('@plugin/token-allocation-adapter', () => ({
  ...jest.requireActual('@plugin/token-allocation-adapter'),
