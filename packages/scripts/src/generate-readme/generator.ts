import { balance } from '@plugin/ea-factories'
import { Adapter } from '@plugin/external-adapter-framework/adapter'
import { SettingsDefinitionMap } from '@plugin/external-adapter-framework/config'
import { InputParameters as V3InputParameters } from '@plugin/external-adapter-framework/validation'
import { InputParametersDefinition } from '@plugin/external-adapter-framework/validation/input-params'
import fs from 'fs'
...
export class ReadmeGenerator {
  ...
  constructor(adapter: WorkspaceAdapter, verbose = false) {
    ...
    if (packageJson.dependencies) {
      const adapterEA = packageJson.dependencies['@plugin/external-adapter-framework']
      if (adapterEA) {
        this.frameworkVersion = 'v3'
        const generatorEA = generatorPack.dependencies['@plugin/external-adapter-framework']
    ...
    this.versionBadgeUrl = `https://img.shields.io/github/package-json/v/goplugin/external-adapters-js?filename=${packagePath}`
