...
export const HELM_CHART_DIR = 'plugin/cl-adapter'
...
IMAGE_REPOSITORY - The docker image reposoitory where the image you want deployed lives. Defaults to the public plugin ecr.`
...
export const checkArgs = (): Inputs => {
  ...
  // Get path to secrets file in goplugin/adapter-secrets
...
export const deployAdapter = (config: Inputs): void => {
  ...
  if (!process.env['USE_HELM_CACHE']) {
    ...
    if (pullHelmChart.code !== 0) {
      process.exitCode = 1
      throw red.bold(
        `Failed to pull the plugin helm chart repository: ${pullHelmChart.toString()}`,
      )
    }
  }
