...
LATEST_VERSION=$(npm view @plugin/external-adapter-framework version)''
...
function bump() (
  ...
  if jq -e "if (.dependencies.\"@plugin/external-adapter-framework\" != null and .dependencies.\"@plugin/external-adapter-framework\" != \"$LATEST_VERSION\") then true else false end" package.json > /dev/null ; then
    current_version=$(jq -r '.dependencies."@plugin/external-adapter-framework"' package.json) 
    echo "Bumping dependency for package $package from $current_version to $LATEST_VERSION"
    jq ".dependencies.\"@plugin/external-adapter-framework\" = \"$LATEST_VERSION\"" package.json > package.json.tmp
    mv package.json.tmp package.json
    changed=true
  fi

  if jq -e "if (.devDependencies.\"@plugin/external-adapter-framework\" != null and .devDependencies.\"@plugin/external-adapter-framework\" != \"$LATEST_VERSION\") then true else false end" package.json > /dev/null ; then
    current_version=$(jq -r '.devDependencies."@plugin/external-adapter-framework"' package.json) 
    echo "Bumping dev dependency for package $package from $current_version to $LATEST_VERSION"
    jq ".devDependencies.\"@plugin/external-adapter-framework\" = \"$LATEST_VERSION\"" package.json > package.json.tmp
    mv package.json.tmp package.json
    changed=true
  fi
