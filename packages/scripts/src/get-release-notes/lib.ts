...
export function getReleaseNotes(): string {
  ...
  changesetFileDirs.map((fileDir) => {
    ...
    const changedAdapters = fileContents.filter((line) => line.indexOf('@plugin') !== -1)

    changedAdapters.map((line) => {
      const adapterNameDirty = line.match(/'@plugin\/.+'/)
      ...
      const adapterNameWithoutPrefix = adapterName.replace('@plugin/', '')
