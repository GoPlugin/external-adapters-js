...
} from '@plugin/external-adapter-framework/config'
...
export async function flattenAllSchemas(): Promise<FlattenedSchema[]> {
  const resolve = createPluginLabsResolver()
    ...
    workspacePackages
      ...
      .map(async (p) => {
        ...
        if (p.framework === '3') {
          ...
          schema = {
            ...schema,
            $id: 'https://external-adapters.pluginlabs.com/schemas/framework.json',
            type: 'object',
            properties: reduced,
          }
        } else if (environment) {
...
/**
 * Create a "json-schema-ref-parser" compatible resolver that matches URLs for https://external-adapters.pluginlabs.com and returns internal schemas
 *
 * @returns Resolver for plugin labs schemas
 */
function createPluginLabsResolver() {
  ...
  const resolver: $RefParser.ResolverOptions = {
    order: 1,
    canRead: /^https:\/\/external-adapters.pluginlabs.com/i,
    read: (file, callback) => {
      if (!callback) {
  ...
  return { pluginLabsResolver: resolver }
}
 ...
 *
 * @param collisionNamespace A namespace like "https://external-adapters.pluginlabs.com/schemas/genesis-volatility-adapter.json"
