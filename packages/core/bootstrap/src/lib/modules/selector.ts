...
const findSupportedEndpoint = <C extends Config, D extends AdapterData>(
  ...
  for (const apiEndpoint of Object.values(apiEndpoints)) {
    // Iterate through supported endpoints of a given Plugin endpoint
    for (const supportedPluginEndpoint of apiEndpoint.supportedEndpoints) {
      if (supportedPluginEndpoint.toLowerCase() === endpoint.toLowerCase()) {
        return apiEndpoint
      }
    }
  }
