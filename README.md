# Plugin External Adapters (TypeScript)

This repository contains the source code for Plugin external adapters. If you would like to contribute, please see the [Contributing](./CONTRIBUTING.md) page for more details.
...
Every External Adapter has some optional environment variables for customizing behavior and turning on advanced features. The list of all available options can be seen [here](https://github.com/goplugin/ea-framework-js/blob/main/docs/reference-tables/ea-settings.md).
...
coincodex-adapter:
  ...
  build:
    ...
    args:
      ...
      package: '@plugin/coincodex-adapter'
    labels:
      com.pluginlabs.external-adapter-type: sources
  environment:
    - EA_PORT=${EA_PORT}
```
...
Images are being published to Plugin's public AWS ECR repositories:
`public.ecr.aws/plugin/adapters`

They can also be found in the public gallery, the registry name is `plugin` (e.g. `https://gallery.ecr.aws/plugin/adapters/1forge-adapter`).
...
```sh
docker pull public.ecr.aws/plugin/adapters/1forge-adapter:latest
```

To run the image use the [docker run command](https://docs.docker.com/engine/reference/run/). For example:

```sh
docker run -p 8080:8080 -e API_KEY='YOUR_API_KEY' public.ecr.aws/plugin/adapters/1forge-adapter:latest
```

It can be helpful to pass a text file to the container to handle giving multiple environment variables:

```sh
docker run -p 8080:8080 --env-file=[[path to your env file]] public.ecr.aws/plugin/adapters/1forge-adapter:latest
```
...
Please refer to [ea-framework-js docs](https://github.com/goplugin/ea-framework-js/tree/main/docs) for topics like performance, rate limiting, caching, overrides and other advanced features.
